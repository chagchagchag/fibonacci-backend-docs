...
### nodeport 정의
포트 매핑
- `호스트 PC → [클러스터 30009 : NodePort 30009] → [argocd-server 80:8080]` 
...
`argocd-server-nodeport` 라는 이름의 NodePort 리소스를 정의한 리소스 정의 파일입니다.
```yaml
---
apiVersion: v1
kind: Service
...
## ArgoCD 생성 스크립트
여기까지 작성한 모든 ArgoCD 생성 스크립트는 `cluster/setup-argocd.sh` 파일 내에 정의했습니다.
`cluster/setup-argocd.sh`
```bash
echo ""
